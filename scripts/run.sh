...
for model in "${MODELS[@]}"; do
    ...
    echo "Running model: $model in orcale doc setting"
    python3 -u ./scripts/get_qa_responses.py \
    --input-path qa_data/nq-open-all_oracle_docs.jsonl.gz \
    --max-new-tokens 100 \
    --num-gpus 2 \
    --model $model \
    --add_system_prompt \
    --output-path qa_predictions/nq-open-oracle-${model}-all_oracle_docs-predictions.jsonl.gz

    python3 -u scripts/evaluate_qa_responses.py \
        --input-path qa_predictions/nq-open-oracle-${model}-all_oracle_docs-predictions.jsonl.gz \
        --output-path qa_predictions/nq-open-oracle-${model}-all_oracle_docs-predictions-scored.jsonl.gz
